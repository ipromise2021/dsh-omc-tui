...
| 版本 | 日期 | 说明 |
| :--- | :--- | :--- |
| **Unreleased** | — | **会话导出安全交互**。`/export` 打开导出面板，默认使用 `$DSH_HOME/exports/<项目名>/`（未设置时为 `~/.dsh/exports/`）并在首次确认时创建；可在面板内编辑相对或绝对目录，自定义目录则严格校验存在、类型与可写性；写入前显示内容范围、目标文件与二次确认，并以仅当前用户可访问权限创建默认目录和导出文件，避免覆盖已有文件。 |
