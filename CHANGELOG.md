...
| 版本 | 日期 | 说明 |
| :--- | :--- | :--- |
| **v0.2.6** | 2026-08-27 | **双模态视觉路由直通、Retina 截图超限自适应缩放与零延迟纯文本发送优化**。新增 `resolveModelVisionSupport` 双模态分流引擎，基于 `inputModalities`/`capabilities.vision` 判定原生视觉模型并直传 `{ type: 'image' }` 图像块，纯代码/文本模型（如 DeepSeek）无缝保持旁路 `analyze_image` Sidecar 自主调度；重构消息提交链路，仅在包含图片时按需检索模型目录，纯文本消息 0ms 零阻塞发送；新增 `downscaleImageBuffer` 跨平台高分屏缩放引擎（macOS `sips`，Linux/Windows `magick`/`convert`/`gm`，2048px 安全基准线），原生实现 `jpegDimensions` 与 `imageDimensions` 保持精确 MIME 与尺寸；缩放后自动同步 dimensions、重新生成 base64 并清除未缩放原图磁盘路径，消除数据不一致；增加真实 4000×3000 Fixture 深度回归单测。 |
