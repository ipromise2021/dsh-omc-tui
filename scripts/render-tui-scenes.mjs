...
const sceneApprovalCard = [
  ...
  ``,
  `${ANSI.coral}│${ANSI.reset} ${ANSI.coral}! approval needed · edit${ANSI.reset}`,
  `${ANSI.coral}│${ANSI.reset} ${ANSI.dim}file${ANSI.reset} src/renderer/diff.js`,
  `${ANSI.coral}│${ANSI.reset}${ANSI.coral}- export function approvalDiffLines(request, args, columns, ANSI = defaultAnsi) {${ANSI.reset}`,
  `${ANSI.coral}│${ANSI.reset}${ANSI.blue}+ export function approvalDiffLines(request, argsOrColumns, columnsOrAnsi, ANSI = defaultAnsi) {${ANSI.reset}`,
  `${ANSI.coral}│${ANSI.reset}${ANSI.blue}+   let args = typeof argsOrColumns === 'object' && argsOrColumns !== null ? argsOrColumns : undefined${ANSI.reset}`,
  `${ANSI.coral}│${ANSI.reset}${ANSI.blue}+   let columns = typeof argsOrColumns === 'number' ? argsOrColumns : (typeof columnsOrAnsi === 'number' ? columnsOrAnsi : 80)${ANSI.reset}`,
  `${ANSI.coral}│${ANSI.reset}  ${ANSI.coral}Y · allow once${ANSI.reset}   ${ANSI.dim}N · deny   Esc · deny${ANSI.reset}`,
  `${ANSI.coral}│${ANSI.reset}  ${ANSI.dim}←→ choose  ·  Enter confirm  ·  y/n also work${ANSI.reset}`,
  ``,
  `${ANSI.rule}──────────────────────────────────────────────────────────────────────────────────────────${ANSI.reset}`,
  `${ANSI.coral}❯${ANSI.reset} `,
  `${ANSI.rule}──────────────────────────────────────────────────────────────────────────────────────────${ANSI.reset}`,
