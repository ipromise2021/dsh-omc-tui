...
export function renderStatusRows(options) {
  ...
  const title = truncateWidth(compactFileReferenceTitle(sessionTitle(sessionEvents)), Math.max(16, effectiveColumns - 72))
  ...
  const permRow = `  ${ANSI.blue}▶▶${ANSI.reset} ${ANSI.muted}permission${ANSI.reset} ${ANSI.blue}${permissionName ?? 'custom'}${ANSI.reset}${ANSI.dim} · Shift+Tab${ANSI.reset}`
